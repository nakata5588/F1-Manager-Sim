...
function playerManagesCommercial(saveWorld, teamId, configured = []) {
  ...
  if (!controlled.has(String(teamId))) return false;
  return responsibilityOwner(saveWorld, String(teamId), "commercial") === "manager";
}

function aiManagesCommercial(saveWorld, teamId, configured = []) {
  const controlled = controlledTeamSet(saveWorld, configured);
  if (!controlled.has(String(teamId))) return true;
  return responsibilityOwner(saveWorld, String(teamId), "commercial") === "delegated";
}
...
function activityEvents(saveWorld, event, configured) {
  ...
  if (![3, 6, 9, 11].includes(month)) return output;
      ...
      if (playerManagesCommercial(saveWorld, teamId, configured)) {
        ...
        output.push({
          ...
          payload: {
            ...
            source: aiManagesCommercial(saveWorld, teamId, configured) ? "delegated_or_ai" : "simulation",
          },
        });
      }
    }
...
function aiCommercialDeals(saveWorld, event, configured) {
  ...
  for (const teamId of teamIds(saveWorld)) {
    ...
    if (!tier) continue;
    const market = listSponsorMarket(saveWorld, teamId, { tier }).filter((row) => !row.categoryConflict && row.interest.score >= 46);
    if (!market.length) continue;
    const rng = createRng(`${saveWorld.meta?.seed}|ai-commercial|${teamId}|${event.date}|${tier}`);
    const pool = market.slice(0, Math.min(4, market.length));
    const candidate = pool[Math.floor(rng.next() * pool.length)] ?? pool[0];
    try {
      ...
      if (raw.type === COMMERCIAL_EVENT.NEGOTIATION_COUNTERED) acceptSponsorCounterEvent(saveWorld, negotiation.id);
      ...
      // A failed AI approach is simply a market outcome; it must not interrupt time progression.
    }
  }
  return output;
}
...
function raceEvents(saveWorld, event) {
  const consequences = raceCommercialConsequences(saveWorld, event.payload?.classification ?? [], event.date);
  const output = [];
...
export function createCommercialManagementSystem(options = {}) {
  ...
  return {
    ...
    eventTypes: [
      ...
      COMMERCIAL_EVENT.NEGOTIATION_ACCEPTED,
    ],
    handle({ saveWorld, event }) {
      if (event.type === SIM_EVENT.CAREER_STARTED) {
        const initialized = initializeCommercialWorld(saveWorld, event.date);
        return { type: COMMERCIAL_EVENT.INITIALIZED, payload: initialized };
      }
      ...
      for (const teamId of teamIds(saveWorld)) {
        ...
        if (controlledTeamSet(saveWorld, configured).has(teamId) && Math.abs(update.delta) >= 0.01) {
          output.push({ type: COMMERCIAL_EVENT.MARKETABILITY_UPDATED, payload: { team_id: teamId, marketability: update.marketability, delta: update.delta } });
        }
      }
      for (const deal of expireCommercialDeals(saveWorld, event.date)) {
        output.push({ type: COMMERCIAL_EVENT.DEAL_EXPIRED, payload: { deal_id: deal.id, team_id: deal.teamId, sponsor_id: deal.sponsorId, sponsor_name: deal.sponsorName } });
      }

...
function replace(path, before, after) {
  ...
  if (!source.includes(before)) throw new Error(`Expected patch anchor not found in ${path}: ${before.slice(0, 120)}`);
...
// ---------------------------------------------------------------------------
{
  const path = "src/game/management/staffRecruitment.js";
  replace(path,
...
// ---------------------------------------------------------------------------
// 2) Era-aware facilities. Simulator is unavailable until explicit future-era
//    technology metadata unlocks it; level zero remains zero, not level one.
// ---------------------------------------------------------------------------
{
  const path = "src/game/management/technical.js";
  replace(path,
...
// AI/delegated departments must obey the exact same facility-era boundary.
{
  const path = "src/sim/systems/teamDevelopment.js";
  replace(path,
...
// Player-facing facility state: locked future technology is visible but cannot
// be built/upgraded, while era-available level-zero facilities show as unbuilt.
{
  const path = "playtest/technical.js";
  replace(path,
...
`function renderFacilities() {
  ...
  return \`<section class="technical-card wide"><span class="management-category">Infrastructure</span><h2>Facilities</h2><div class="facility-grid">\${rows.map((row) => {
    ...
    const availabilityText = locked
      ...
      : status === "upgrading" ? "Upgrade in progress" : escapeHtml(label(status));
    const action = row.upgrade
      ...
      : technical.responsibility === "manager" && !locked && Number(row.level ?? 0) < 10
        ...
        : "";
    return \`<article class="facility-card"><strong>\${escapeHtml(row.label)}</strong><div>\${levelText}</div><div class="technical-muted">\${availabilityText}</div><div class="technical-muted">\${escapeHtml(row.source)}</div>\${action}</article>\`;
  }).join("") || '<div class="technical-muted">No facilities recorded for this team.</div>'}</div></section>\`;
}
`);
}
...
// ---------------------------------------------------------------------------
// 3) 1980 tyre supplier packages become usable derived gameplay compound
//    families. No modern Soft/Medium/Hard historical claim is introduced.
// ---------------------------------------------------------------------------
{
  const path = "src/sim/raceStrategy.js";
  replace(path,
...
// ---------------------------------------------------------------------------
// Regression coverage for all three playtest corrections.
// ---------------------------------------------------------------------------
...
console.log("Playtest pass 02 patches applied.");

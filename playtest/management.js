...
function statusPill(value) {
  ...
  return `<span class="status-pill status-${escapeHtml(String(value ?? "unknown"))}">${escapeHtml(label)}</span>`;
}

function enumSelect(id, currentValue, options = []) {
  const values = [...new Set([currentValue, ...options].filter(Boolean))];
  return `<select id="${escapeHtml(id)}">${values.map((value) => `<option value="${escapeHtml(value)}" ${value === currentValue ? "selected" : ""}>${escapeHtml(publicLabel(value))}</option>`).join("")}</select>`;
}
...
function renderBoard() {
  ...
  ${board.requests.length ? `<div class="management-table-wrap compact-table"><table><thead><tr><th>Request</th><th>Status</th><th>Submitted</th><th>Outcome</th></tr></thead><tbody>${board.requests.slice().reverse().map((row) => `<tr><td>${escapeHtml(publicLabel(row.kind))}</td><td>${statusPill(row.status)}</td><td>${humanDate(row.submittedAt)}</td><td>${escapeHtml(row.value ?? row.reason ?? "Pending")}</td></tr>`).join("")}</tbody></table></div>` : ""}`;
}
...
function renderCareer() {
  ...
  <div class="management-table-wrap"><table><thead><tr><th>Team</th><th>Reputation</th><th>Career fit</th><th></th></tr></thead><tbody>${managerCareer.vacancies.map((row) => `<tr><td><strong>${entityLink("team", row.id, row.name)}</strong></td><td>${Math.round(row.reputation)}</td><td>${Math.abs(Number(row.reputation) - Number(career.reputation ?? 0)) <= 15 ? "Realistic" : "Stretch"}</td><td><button data-manager-apply="${escapeHtml(row.id)}">Apply</button></td></tr>`).join("")}</tbody></table></div>
  <div class="management-section-title"><div><span class="management-category">Record</span><h2>Career history</h2></div></div>
  <div class="career-timeline">${(career.history ?? []).slice().reverse().map((row) => `<div><strong>${escapeHtml(publicLabel(row.type))}</strong><span>${escapeHtml(row.teamId ?? row.reason ?? "")}</span><time>${humanDate(row.date)}</time></div>`).join("") || '<div class="management-empty">No career history yet.</div>'}</div>`;
}
...
function staffEditor() {
  ...
  return `<article class="management-contract-editor"><div class="eyebrow">Staff negotiation</div><h2>${entityLink("staff", selectedStaffNegotiation.staffId, selectedStaffNegotiation.staffName)}</h2><p class="muted">${escapeHtml(publicLabel(selectedStaffNegotiation.role, "Role"))} · start ${selectedStaffNegotiation.startSeason} · expires ${humanDate(selectedStaffNegotiation.expiresAt)}</p>${isCounter ? '<div class="management-notice">A staff counter-offer is waiting in your Inbox.</div>' : `<div class="management-form-grid">${compensationFields(terms, "staff-offer")}<label>Contract length<input id="staff-offer-length" type="number" min="1" max="5" value="${terms.lengthYears}"></label><label>Role${enumSelect("staff-offer-role", terms.role, ["technical_director", "sporting_director", "team_principal", "race_engineer", "chief_designer", "head_of_aerodynamics", "chief_mechanic", "performance_engineer", "strategy_engineer", "scout"])}</label></div><div class="management-actions"><button class="primary" data-submit-staff-offer="${escapeHtml(selectedStaffNegotiation.id)}">Submit offer</button><button data-withdraw-staff="${escapeHtml(selectedStaffNegotiation.id)}">Withdraw</button></div>`}</article>`;
}

function renderStaff() {
  return `${staffEditor()}<div class="management-toolbar staff-toolbar"><input id="staff-query" placeholder="Search staff or role" value="${escapeHtml(queryFrom(staffRequest))}"><button data-action="search-staff">Search</button><button data-action="show-all-staff">All staff</button></div><div class="management-table-wrap"><table><thead><tr><th>Staff</th><th>Role</th><th>Team</th><th>Ability</th><th>Technical</th><th>Leadership</th><th>Strategy</th><th>Scouting</th><th>Interest</th><th></th></tr></thead><tbody>${staffRecruitment.candidates.map((row) => `<tr><td><strong>${entityLink("staff", row.id, row.name)}</strong><div class="muted small">${escapeHtml(row.nationality ?? "—")}</div></td><td>${escapeHtml(publicLabel(row.role, "Role"))}</td><td>${escapeHtml(row.currentTeamId ?? "Free agent")}</td><td>${Math.round(row.ability)}</td><td>${row.technical ?? "—"}</td><td>${row.leadership ?? "—"}</td><td>${row.strategy ?? "—"}</td><td>${row.scouting ?? "—"}</td><td>${staffInterest(row)}</td><td><button class="primary" data-negotiate-staff="${escapeHtml(row.id)}">Negotiate</button></td></tr>`).join("")}</tbody></table></div><div class="management-section-title"><div><span class="management-category">Negotiations</span><h2>Staff contracts</h2></div></div><div class="management-table-wrap compact-table"><table><thead><tr><th>Staff</th><th>Status</th><th>Start</th><th>Role</th><th>Terms</th></tr></thead><tbody>${staffContracts.negotiations.map((row) => `<tr data-staff-contract-row="${escapeHtml(row.id)}" class="${selectedStaffNegotiation?.id === row.id ? "selected-row" : ""}"><td>${entityLink("staff", row.staffId, row.staffName)}</td><td>${escapeHtml(publicLabel(row.status, "Status"))}</td><td>${row.startSeason}</td><td>${escapeHtml(publicLabel(row.role, "Role"))}</td><td>${escapeHtml(publicLabel(row.expectedTerms.compensationMode, "Compensation"))}</td></tr>`).join("")}</tbody></table></div>`;
}
...
function renderMarket() {
  ...
  return `<div class="management-stack">${market.offers.map((row) => `<article class="management-message"><div class="management-message-head"><div><span class="management-category">${escapeHtml(row.source)}</span><h3>${escapeHtml(row.driverName)}</h3></div><span class="muted">expires ${humanDate(row.expiresAt)}</span></div><p><strong>${entityLink("team", row.teamId, row.teamName ?? row.teamId)}</strong> · future season ${row.startSeason}</p><div class="negotiation-context"><strong>Driver interest</strong><span>${Math.round(row.interest?.score ?? 0)}/100 · ${escapeHtml(publicLabel(row.interest?.level, "Unknown"))}</span></div></article>`).join("")}</div>`;
}
...
function sponsorInterest(row) {
  return `<strong class="interest ${escapeHtml(row.interest?.level ?? "open")}">${escapeHtml(publicLabel(row.interest?.level ?? "unknown", "Unknown"))}</strong><div class="muted small">${Math.round(row.interest?.score ?? 0)}/100</div>`;
}
...
function renderCommercial() {
  ...
  const crisis = windowFinances?.crisis ?? null;
  const crisisSummary = crisis && !["stable", "watch"].includes(crisis.stage) ? `<div class="management-section-title"><div><span class="management-category">Financial crisis</span><h2>${escapeHtml(publicLabel(crisis.stage, "Crisis"))}</h2></div></div>
  <section class="finance-summary-grid">
    <article><span>Crisis stage</span><strong>${escapeHtml(publicLabel(crisis.stage, "Crisis"))}</strong><small>Since ${humanDate(crisis.stageSince)}</small></article>
    <article><span>Debt principal</span><strong>${money(crisis.debtPrincipal)}</strong><small>${Math.round(Number(crisis.debtAnnualRate ?? 0) * 100)}% annual rate</small></article>
    <article><span>Debt service</span><strong>${money(crisis.debtService?.total)}</strong><small>Monthly interest + principal</small></article>
    <article><span>Ownership</span><strong>${escapeHtml(publicLabel(crisis.owner?.model, "Ownership"))}</strong><small>Support ${Math.round(crisis.owner?.supportIndex ?? 0)}/100 · patience ${Math.round(crisis.owner?.patienceIndex ?? 0)}/100</small></article>
    <article><span>Distress duration</span><strong>${crisis.distressMonths ?? 0} mo</strong><small>${crisis.negativeCashMonths ?? 0} months below zero cash</small></article>
    <article><span>Spending freeze</span><strong>${crisis.spendingFreeze ? "Active" : "No"}</strong><small>${escapeHtml(publicLabel(crisis.freezeReason, "No restriction"))}</small></article>
  </section>
  <div class="management-notice"><strong>Crisis authority:</strong> Employment contracts and Race Entry remain separate. New discretionary commitments can be blocked while the Board, ownership and creditors pursue a rescue. Player choices arrive through the Inbox.</div>` : "";
  ...
  ${commercial.negotiations.length ? `<div class="management-table-wrap compact-table"><table><thead><tr><th>Sponsor</th><th>Tier</th><th>Status</th><th>Interest</th><th>Offers</th><th>Expires</th></tr></thead><tbody>${commercial.negotiations.map((row) => `<tr data-sponsor-negotiation-row="${escapeHtml(row.id)}" class="${selectedSponsorNegotiation?.id === row.id ? "selected-row" : ""}"><td>${escapeHtml(row.sponsorName)}</td><td>${escapeHtml(publicLabel(row.tier, "Tier"))}</td><td>${escapeHtml(publicLabel(row.status, "Status"))}</td><td>${Math.round(row.interest?.score ?? 0)}/100</td><td>${row.offers?.length ?? 0}</td><td>${humanDate(row.expiresAt)}</td></tr>`).join("")}</tbody></table></div>` : '<div class="management-empty">No sponsor negotiations yet.</div>'}`;
}
...
function render() {
  ...
  if (careerState.screen === "new_career") {
    root.innerHTML = '<div class="management-standalone"><div><h1>No active career</h1><p>Start a career from the Main Menu first.</p><a class="management-link-button" href="/">Return to Career Setup</a></div></div>';
    return;
  }

...
function renderDriverContracts() {
  ...
  return `${editor}<div class="management-table-wrap"><table><thead><tr><th>Driver</th><th>Status</th><th>Start</th><th>Offers</th><th>Interest</th><th>Release</th><th>Terms</th></tr></thead><tbody>${contracts.negotiations.map((row) => `<tr data-contract-row="${escapeHtml(row.id)}" class="${selectedNegotiation?.id === row.id ? "selected-row" : ""}"><td><strong>${entityLink("driver", row.driverId, row.driverName)}</strong></td><td>${escapeHtml(row.status)}</td><td>${row.startSeason}</td><td>${row.offers.length}</td><td>${escapeHtml(row.interest?.level?.replaceAll("_", " ") ?? "—")}</td><td>${row.transferCompensation?.required ? `${escapeHtml(row.transferCompensation.mode)} ${escapeHtml(row.transferCompensation.value)}` : "None"}</td><td>${escapeHtml(row.expectedTerms.compensationMode)}</td></tr>`).join("")}</tbody></table></div>`;
}
